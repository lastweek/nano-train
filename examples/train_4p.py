...
def _build_model_for_context(ctx: RuntimeContext) -> DeepSeekModel:
    ...
    return DeepSeekModel(model_config, parallel_context=parallel_context).to(parallel.device)
...
def _log_training_start(ctx: RuntimeContext, model: DeepSeekModel) -> None:
...
class Train4PBootstrap(RuntimeBootstrap):
    ...
    def build_context(self, args: argparse.Namespace) -> RuntimeContext:
        ...
        return RuntimeContext(
            ...
            run_config=RunConfig(args=args, pp_layer_splits=pp_layer_splits),
        )
...
class Train4POptimizerRuntime:
    ...
    def initialize(self, model: torch.nn.Module, ctx: RuntimeContext) -> OptimizerState:
        ...
        optimizer = _build_optimizer(model=model, shard_info=shard_info, ctx=ctx)
        _log_training_start(ctx, model)
        return OptimizerState(optimizer=optimizer, shard_info=shard_info)
