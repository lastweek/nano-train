# Documentation Index

**Purpose**: Central index for nano-train documentation, organized by learning path and
operational task.

**Audience**: Engineers learning distributed training and contributors working on infra paths.

**Prerequisites**: Basic PyTorch familiarity and ability to run scripts under `examples/`.

**Related Docs**:
- [Project README](../README.md)

## Start Here

1. [TP + DP Communication Guide](tp_dp_communication.md)
2. [TP + EP + DP Communication Guide](ep_tp_dp_communication.md)
3. [TP + PP + EP + DP Communication Guide](pp_tp_ep_dp_communication.md)
4. [DeepSeekMoE Auxiliary Losses](deepseek_moe_aux_losses.md)
5. [EP Training Monitoring Quickstart](training_monitoring_quickstart.md)

## Learning Guides

### Parallelism

- [TP + DP Communication Guide](tp_dp_communication.md)
- [TP + EP + DP Communication Guide](ep_tp_dp_communication.md)
- [TP + PP + EP + DP Communication Guide](pp_tp_ep_dp_communication.md)
- [DeepSeekMoE Auxiliary Losses](deepseek_moe_aux_losses.md)

## Operations

- [Logging Guide](logging.md)
- [Training Monitoring Quickstart](training_monitoring_quickstart.md)
- [Training Monitoring Metrics Reference](training_monitoring_metrics_reference.md)

## Reference

- [Model Info Utility](model_info.md)
- [Model Info Appendix](model_info_appendix.md)

## Historical Artifact (PDF)

- [LLM Training Diagnose PDF](llm-training-diagonose.pdf)
