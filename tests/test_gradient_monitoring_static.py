...
def _find_trainer_training_step(tree: ast.AST) -> ast.FunctionDef:
    """Find `Trainer.training_step` in the parsed module AST."""
        ...
        if isinstance(node, ast.ClassDef) and node.name == "Trainer":
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and item.name == "training_step":
                    return item
    raise AssertionError("Could not find Trainer.training_step() in src/trainer.py")
...
class _TrainingStepCallCollector(ast.NodeVisitor):
...
def test_gradient_monitoring_logged_before_zero_grad() -> None:
    ...
    training_step = _find_trainer_training_step(tree)

    collector = _TrainingStepCallCollector()
    collector.visit(training_step)

    def _min_line(name: str) -> int:
        if not collector.calls[name]:
            raise AssertionError(f"Expected call to {name} in Trainer.training_step()")
        return min(call.lineno for call in collector.calls[name])

    zero_grad_line = _min_line("self.optimizer.zero_grad")
    ...
    for name in [
        ...
        unguarded = [
            call.lineno for call in collector.calls[name] if not call.guarded_by_log_this_step
        ]
    ...
    unguarded_hist = [
        call.lineno
        for call in collector.calls["self._log_gradient_histograms"]
